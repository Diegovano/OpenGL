...
ShapeData ShapeGenerator::MakeTriangle()
{
	...
	ret.numIndices = NUM_ARRAY_ELEMENTS(myTri);
...
ShapeData ShapeGenerator::MakePlaneVerts(glm::uint dimensions)
{
	ShapeData ret;
...
ShapeData ShapeGenerator::MakePlaneIndices(glm::uint dimensions)
{
	ShapeData ret;
...
ShapeData ShapeGenerator::MakePlane(glm::uint dimensions)
{
...
ShapeData ShapeGenerator::MakeTeapot(glm::uint tesselation, const glm::mat4& lidTransform)
{
	...
	for (glm::uint i = 0; i < ret.numVertices; i++)
	{
		...
		v.normal.x = normals[i * 3 + 0];  //had written v.pos for all normals
...
ShapeData ShapeGenerator::GenerateNormals(const ShapeData& data)
{
	...
	ret.indices = new GLushort[ret.numIndices];
	for (unsigned int i = 0; i < ret.numIndices; i++) ret.indices[i] = i;
	return ret;
}